...
import { addUser, forgotPassword, generateToken, getUser, getUserByID, resetPassword, generateActivationToken, activateAccount, generateUserToken, activationMail } from "../Controllers/users.js";
...
router.post("/signup", async (req, res) => {
    try {
      ...
      const hashedUser = await { ...req.body, password: hashedPassword, isActivated:false, activationKey: activationKey };
      ...
      if(!result.acknowledged){
        ...
        transporter.sendMail(mailOptions, function(error, info){
            if (error) {
              ...
              return res.status(400).send({ message:"Error sending email", result:result.acknowledged}); 
            } else {
  ...
  router.post("/activation", async (req, res) => {
    try {
      ...
      if(!user.isActivated){
      const activationKey = Math.random().toString(36).substring(2,9);
      const result = await activationMail(req.body.email, {isActivated:false, activationKey: activationKey});
      ...
      if(!result.lastErrorObject.updatedExisting){
          return res.status(404).json({message:"Error sending activation mail"})
      }
      else {
        transporter.sendMail(mailOptions, function(error, info){
            if (error) {
              console.log("Email not sent",error);
              return res.status(400).send({ message:"Error sending email", result:result.lastErrorObject.updatedExisting}); 
            } else {
              console.log('Email sent: ' + info.response); 
              return res.status(200).send({ result:result.lastErrorObject.updatedExisting, message: "Activation link sent", data:user.email}); 
            }
          });
        }     
      }
      else{
       return res.status(400).json({message:`Account already activated ${user.isActivated}`});
      }
  ...
  router.post("/activate/:id/:token", async (req,res) => {
    try{
      ...
      if(decode.id && req.body.isActivated){ 
        const result = await activateAccount(user.email,{isActivated:true, activationKey: ""}) 
        if(!result.lastErrorObject.updatedExisting){
            return res.status(400).json({message:"Error activating account"})
          }
          else {
            console.log("account activated");
            return res.status(200).json({decode:decode, message:"Account activated", result:result})
          }
      }
      }
      catch(err){
...
router.post("/login", async(req, res) => {
  try {
    ...
    res.status(200).json({ data: {fname: user.fname, lname: user.lname, email: user.email, id: user._id }, token:token });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal server error" });
  }
